...
function classNameIsValid(className) {
  return className === '_User' || className === '_Installation' || className === '_Session' || className === '_SCHEMA' || //TODO: remove this, as _SCHEMA is not a valid class name for storing Parse Objects.
  className === '_Role' || className === '_Product' || joinClassRegex.test(className) ||
...
var Schema = function () {
  ...
  _createClass(Schema, [{
    ...
    value: function reloadData() {
      ...
      this.data = {};
      this.perms = {};
      return this.collection.find({}, {}).toArray().then(function (mongoSchema) {
    ...
    value: function addClassIfNotExists(className, fields) {
      ...
      return this.collection.insertOne(mongoObject.result).then(function (result) {
        return result.ops[0];
    ...
    value: function validateClassName(className, freeze) {
      ...
      return this.collection.insert([{ _id: className }]).then(function () {
        ...
        return _this2.validateClassName(className, true);
      }, function (error) {
    ...
    value: function validateField(className, key, type, freeze) {
      ...
      return this.collection.findAndModify(query, {}, update, {}).then(function () {
        ...
        // Ensure that the schema now validates
        return _this4.validateField(className, key, type, true);
      }, function (error) {
        // The schema still doesn't validate. Give up
    ...
    value: function deleteField(fieldName, className, database) {
      ...
      return this.reloadData().then(function () {
        return _this5.hasClass(className).then(function (hasClass) {
          if (!hasClass) {
            throw new Parse.Error(Parse.Error.INVALID_CLASS_NAME, 'Class ' + className + ' does not exist.');
          }
          if (!_this5.data[className][fieldName]) {
            throw new Parse.Error(255, 'Field ' + fieldName + ' does not exist, cannot delete.');
          }

          if (_this5.data[className][fieldName].startsWith('relation<')) {
          ...
          return database.collection(className).then(function (collection) {
            var mongoFieldName = _this5.data[className][fieldName].startsWith('*') ? '_p_' + fieldName : fieldName;
            return collection.update({}, { "$unset": _defineProperty({}, mongoFieldName, null) }, { multi: true });
          });
        })
        // Save the _SCHEMA object
        .then(function () {
          return _this5.collection.update({ _id: className }, { $unset: _defineProperty({}, fieldName, null) });
        });
      });
    }
