...
var UserController = exports.UserController = function (_AdaptableController) {
  ...
  _createClass(UserController, [{
    ...
    value: function setPasswordResetToken(email) {
      var token = (0, _cryptoUtils.randomString)(25);
      return this.config.database.adaptiveCollection('_User').then(function (collection) {
        // Need direct database access because verification token is not a parse field
        return collection.findOneAndUpdate({ email: email }, // query
    ...
    value: function updatePassword(username, token, password, config) {
      ...
      return this.checkResetTokenValidity(username, token).then(function (user) {
        return updateUserPassword(user._id, password, _this4.config);
      }).then(function () {
        // clear reset password token
        return _this4.config.database.adaptiveCollection('_User').then(function (collection) {
          // Need direct database access because verification token is not a parse field
